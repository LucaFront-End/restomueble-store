...
async function getProducts(collectionId?: string) {
    try {
        ...
        if (collectionId) {
            return items.filter((p) => p.collectionIds?.includes(collectionId));
        }
        return items;
    } catch (error) {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    if (!collection) return { title: "Colección no encontrada | Josepja" };
    return {
...
export default async function TiendaCategoriaPage({ params }: PageProps) {
    const { categoria } = await params;
    const collection = getCollectionBySlug(categoria);
        ...
        <main className="bg-white min-h-screen">
            ...
            <div className="container mx-auto max-w-7xl px-6 pt-8">
                <nav className="flex items-center gap-2 text-xs font-medium uppercase tracking-widest text-gray-400">
                    ...
                    <span>→</span>
                    <Link href="/tienda" className="hover:text-[var(--accent)] transition-colors">Tienda</Link>
                    <span>→</span>
